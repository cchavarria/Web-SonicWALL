...
function init() {
		...
		filterMap = [
			...
			},
			{
				...
				callback: function (title) {
					...
					$(this).multipleSelect({
						placeholder: this.data('selected'),
						minimumCountSelected: 0,
						countSelected: this.data('selected') + '&nbsp;(#)',
	...
	$(document).ready(function () {
		...
		getLocalizedContent(['EventLabelDate','EventLabelEventType', 'LabelAllEventsSelected', 'LabelSelectAllEvents', 'EventLabelDate', 'EventLabelRecordedDate']).done(function() {
...
function populateListing(clear) {
	...
	$.ajax({
		...
		$.each(dataopt.data, function (key, val) {
			...
			var htmlFragment = '<div class="col-md-3 col-sm-4 col-xs-12"> ' +
				'<a href="' + val.url + '">' +
		...
		if (dataopt.total) {
			...
			if(dataopt.total > entriesPerType[pageType].total * page) {
...
function getDataSet(incrementPage) {
	...
	$.each(mapping, function(key, id) {
		...
		if(id == 'event_type') {
			...
			dataset[key] = val.join(',') + '|' + entriesPerType[pageType];
		}
