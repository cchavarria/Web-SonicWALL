...
function init() {
	...
	function populateDropdowns(dropdown, data, callback) {
		...
		return $.ajax({
			...
			if (dropdown == 'solution') {
				elem.append('<option value="">' + getLocalizedContent('LabelAllSolutions') + '</option>');
			}
			else if(dropdown == 'brand') {
				elem.append('<option value="">' + getLocalizedContent('LabelAllProductLines') + '</option>');
			}
			else if(dropdown == 'product') {
				elem.append('<option value="">' + getLocalizedContent('LabelAllProducts') + '</option>');
			}
	...
	return $.ajax({
		...
		$.each(dataopt.data, function (key, val) {
			var htmlFragment = '<div class="col-md-3 col-sm-4 col-xs-12" style="display: none;"> ' +
				...
				'    <img class="img-responsive center-block" src="' + val.ImageURL2 + '" alt="" onerror="this.style.display=\'none\';" /> ' +
